...
public class ShootingGame extends JFrame {
	...
	private Image enemy6screen = new ImageIcon("src/images/enemy6_screen.jpg").getImage();
	private Image enemy7screen = new ImageIcon("src/images/enemy7_screen.gif").getImage();
	...
	public void screenDraw(Graphics g) {
		if(isMainScreen == true) {//메인 화면 띄우기
			...
			if(game.score < 3500) g.drawImage(gameScreen, 0, 0, null);
			else if(game.score >= 3500 && game.score < 4500) g.drawImage(enemy6screen, 0, 0, null);
			else if(game.score >= 4500) g.drawImage(enemy7screen, 0, 0, null);
			game.gameDraw(g);
			for (int i = 0; i < potionCount; i++) {
				if(game.isEnd == false && game.isOver == false) g.drawImage(potionImage, 260 + (i * 30), 45, null);
			}
			this.repaint();
		}
